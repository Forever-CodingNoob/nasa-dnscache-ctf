...
class DNSForwarderHandler(socketserver.BaseRequestHandler):
    ...
    def handle(self):
        ...
        # Send the response back to the original client WITH DELAY
...
if __name__ == "__main__":
    ...
    with ThreadingUDPServer((HOST, PORT), DNSForwarderHandler) as server:
        print(f"[*] Benign DNS Forwarder running on {HOST}:{PORT}, forwarding to {UPSTREAM_DNS}")
        try:
            server.serve_forever()
        except KeyboardInterrupt:
            print("\n[!] Shutting down Benign DNS Forwarder.")
            server.shutdown()
